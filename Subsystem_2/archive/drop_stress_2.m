...
%% Split data
% Split data sets -----
param = drop_stress_2; % Change depending on parameter of interest
r25 = round(0.25 * 10); 

xtest = samples(1:r25,:,:); 
xtrain = samples(r25+1:end,:,:); 
ytest = param(1:r25,:,:); 
ytrain = param(r25+1:end,:,:); 
...
linreg_rsq = 1 - (sum((sty_test - predicted_sty).^2)/sum((sty_test - mean(sty_test)).^2))
 ...
 %% Linreg 3D plot for 2 variables - Spread & Radius
...
ft = @(b,x) b(1).*x(:,1) + b(2).*x(:,2) + b(3).*x(:,3) + b(4).*x(:,4) + b(5).*x(:,1).*x(:,2).*x(:,3);
%     x(:,4).*(b(5).*x(:,1) + b(6).*x(:,2) + b(7).*x(:,3) + b(8).*x(:,4)) %+ ...
%     x(:,2).*(b(9).*x(:,1) + b(10).*x(:,2) + b(11).*x(:,3) + b(12).*x(:,4)) + ...
%     x(:,3).*(b(13).*x(:,1) + b(14).*x(:,2) + b(15).*x(:,3) + b(16).*x(:,4)) + ...
%     x(:,4).*(b(17).*x(:,1) + b(18).*x(:,2) + b(19).*x(:,3) + b(20).*x(:,4));
B0 = ones(8, 1);
B = nlinfit(stx_train', sty_train', ft, B0)

%% Nonlinreg Assessing R^2
predicted_sty = stx_test*B(1:4) + stx_test(:,1).*stx_test(:,2).*stx_test(:,3)*B(5);
%     (stx_test(:,3).*stx_test)*B(13:16) + ...
%     (stx_test(:,4).*stx_test)*B(17:20);
linreg_rsq = 1 - (sum((sty_test - predicted_sty).^2)/sum((sty_test - mean(sty_test)).^2))

 %% Nonlinreg 3D plot for 2 variables - Spread & Radius
% extracting standardised training data
st_spread_train = stx_train(1,:)'; %'Spread [deg]'
st_radius_train = stx_train(2,:)'; %'Radius [mm]'
st_thickness_train = stx_train(3,:)'; % 'Thickness [mm]'
...
yfit = min(lin_y):0.2:max(lin_y);
[XFIT,YFIT] = meshgrid(xfit,yfit);
ZFIT = B(1)*XFIT + B(2)*YFIT + XFIT.*YFIT*B(5);
mesh(XFIT,YFIT,ZFIT)
view(50,10)
hold off
