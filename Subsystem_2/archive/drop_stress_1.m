...
%% Split data
% Split data sets -----
param = drop_stress_1; % Change depending on parameter of interest
r25 = round(0.25 * 10); 
...
ft = @(b,x) b(1).*x(:,1) + b(2).*x(:,2) + b(3).*x(:,3) + b(4).*x(:,4) + b(5).*x(:,1).*x(:,2);
...
predicted_sty = stx_test*B(1:4) + stx_test(:,1).*stx_test(:,2)*B(5);
